...
| `references/commands.md`       | 各 `/dev-workflow` 子命令的详细处理逻辑（命令权威）                |
...
| `references/summary.md`        | `/dev-workflow summary` 交付清单（DDL / 队列 / API，含 `--merge`） |
...
```
阶段 1 需求讨论        /dev-workflow req create {域}/{父需求名}
                          ↓
                          /dev-workflow req split {域}/{父需求名}   （交互式向导逐个创建子需求 metadata.md）
                          ↓
                                  （产生父需求讨论文档 docs/discuss/{域}/{父需求名}/{父需求名}.md + 子需求 discussion.md）
   ▼
阶段 2 分析与设计      /dev-workflow next {子需求ID}
                          （产生 analysis/、design.md、design-consensus.md、dev-tasks.md）
   ▼
┌────────────────────────────────────────────────┐
│ 阶段 3 设计审核门  ★人工★  PENDING_DESIGN_REVIEW │  ← 清单式审核，缺项打回阶段 2
│   /dev-workflow approve {子需求ID}                           │
└────────────────────────────────────────────────┘
   ▼
阶段 4 开发与逐任务审查   /dev-workflow next {子需求ID}
                          （plan → 编码 → CR → 改写 → 复验 → DONE）
   ▼
阶段 5 收尾验收        /dev-workflow next {子需求ID}
   │  （产生 acceptance.md）
   ▼
COMPLETED ──→ /dev-workflow version ready ... close ... archive
            ──→ /dev-workflow summary {版本号}              →  change-manifest.md（DDL / Job·MQ / API，给 DBA 与前端）
            ──→ /dev-workflow summary --merge {V1} {V2} ...  →  docs/version/.merged/change-manifest-{Y}-{M}-{D}.md

※ 阶段 4/5 若发现设计/需求层缺陷 → /dev-workflow rework {子需求ID} 按根因层级回退

※ 任何时候变更 manifest / CLAUDE.md / docs/workflow/ → /dev-workflow discovery refresh
```
...
| -------------------------- | ----------------------- | ---------------------------------- | --------------------------------------- | ----------------------------------------------------------------------- |
| 设计审核门                 | `PENDING_DESIGN_REVIEW` | `/dev-workflow approve {子需求ID}` | design-consensus 必含清单逐项 ✓         | design-consensus.md 末尾追加 `## 设计确认: APPROVED`，状态置 DEVELOPING |
| 任务 plan 门（仅复杂任务） | `PENDING_PLAN_REVIEW`   | `/dev-workflow approve {子需求ID}` | plan 必含项齐全                         | plan.md 末尾追加 `## 设计确认: APPROVED`，任务状态置 PLAN_CONFIRMED     |
| CR 问题裁决门（逐任务）    | `PENDING_CR_REVIEW`     | `/dev-workflow approve {子需求ID}` | 全部问题标注 ACCEPTED/REJECTED/MODIFIED | 任务状态置 CR_CONFIRMED → 走改写⑤ / 复验⑥                               |
...
| 动作                             | 触发                                       | 读取                                                 | 写入                                                          | 校验                         |
| -------------------------------- | ------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------------------- | ---------------------------- |
| 校验域合法性                     | `/dev-workflow req create {域}/{父需求名}` | `{discovery_cmd}` 域清单 / `docs/workflow/` 模块命名 | —                                                             | 域 ∈ manifest 声明的模块清单 |
| 校验父需求不存在                 | 同上                                       | `docs/discuss/{域}/{父需求名}/` 是否存在             | —                                                             | 不存在才能创建               |
| 创建父需求目录骨架               | 同上                                       | —                                                    | `docs/discuss/{域}/{父需求名}/.task/parent.md`                | —                            |
| 列出已存在子需求（用于重名检查） | `/dev-workflow req split {域}/{父需求名}`  | `.task/` 子目录扫描                                  | —                                                             | 用于交互式向导重名检查       |
| 交互式向导循环                   | 同上                                       | —                                                    | 每个子需求一个目录 + `metadata.md`（8 字段）                  | 描述非空                     |
| 父需求讨论文档                   | 阶段 1                                     | —                                                    | `docs/discuss/{域}/{父需求名}/{父需求名}.md`                  | 含「影响分析」章节           |
| 子需求讨论文档                   | 阶段 1 触发                                | —                                                    | `docs/discuss/{域}/{父需求名}/.task/{子需求名}/discussion.md` | 包含子需求特定讨论           |
...
| 动作                 | 触发                            | 读取                                                                                             | 写入                                                                                     | 校验                                               |
| -------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- | -------------------------------------------------- |
| 提取受影响模块       | `/dev-workflow next {子需求ID}` | 父需求讨论文档 + 子需求 `discussion.md` 影响分析                                                 | `metadata.md` 阶段产物引用表                                                             | —                                                  |
...
| 动作               | 触发                               | 读取                       | 写入                                                                | 校验                             |
| ------------------ | ---------------------------------- | -------------------------- | ------------------------------------------------------------------- | -------------------------------- |
| 定位目标           | `/dev-workflow approve {子需求ID}` | `metadata.md.currentState` | —                                                                   | 必须处于 `PENDING_DESIGN_REVIEW` |
...
| 出 plan（复杂）                  | PLANNING → PENDING_PLAN_REVIEW                 | `architect` / `planner`（只读）    | `design-consensus.md` + `docs/workflow/{模块}` + 现有代码                                                 | `.task/{子需求名}/plans/{序号}-{模块名}.md`                                                               | 6 项齐全（签名 / 数据模型 / 时序 / 边界 / 测试用例 / 迁移） |
...
| ③ CR 扫描                        | CODING → CR_SCANNED                            | `code-reviewer`（只读）            | `git diff <默认分支>...HEAD -- {module_root_glob}` + done.md + design-consensus.md + docs/workflow/{模块} | `.task/{子需求名}/review/{模块名}-{任务序号}.md`（编号清单 + 严重度 + 文件:行 + 建议改法 + 裁决:PENDING） | **绝不允许直接改代码**                                      |
| ④ 人工裁决                       | CR_SCANNED → PENDING_CR_REVIEW                 | 人工                               | 报告『人工裁决』区                                                                                        | 报告『人工裁决』区填 ACCEPTED / REJECTED / MODIFIED                                                       | 全部 PENDING 必须变更为三选一                               |
| 锁定裁决 + CR_CONFIRMED          | PENDING_CR_REVIEW → CR_CONFIRMED               | `/dev-workflow approve {子需求ID}` | —                                                                                                         | `metadata.md` 任务状态置 CR_CONFIRMED                                                                     | —                                                           |
...
- 设计错 / 多任务受牵连 → `/dev-workflow rework {子需求ID}`（设计级，强门禁：version ∈ {DRAFT/IN_PROGRESS/READY}）

### 2.6 收尾：`/dev-workflow summary {版本号}`

| 动作                               | 读取                                                                                             | 写入                                                                                                                               | 校验                                   |
| ---------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| 启动 `document-specialist`（只读） | 版本号对应的所有 `metadata.md` + 关联 `design-consensus.md` / `dev-tasks.md` / `acceptance.md`   | 单域：`docs/discuss/{域}/{父需求名}/.task/.versions/{版本号}/change-manifest.md`；跨域：`docs/version/{版本号}/change-manifest.md` | version ∈ {IN_PROGRESS/READY/RELEASED} |
...
### 2.7 多版本合并：`/dev-workflow summary --merge {版本A} {版本B} ...`
...
### 2.8 返工：`/dev-workflow rework {子需求ID}`
...
### 2.9 发现失效：`/dev-workflow discovery refresh`
...
| --------------------------- | ------------------------------------------------------------- | ---------------------------------------------- | ----------------------------------------------------------------------------------------- | --------------------- | ---------------------------- |
| N1 创建父需求               | `/dev-workflow req create {域}/{父需求名}`                    | manifest 模块清单                              | `docs/discuss/{域}/{父需求名}/.task/parent.md`                                            | —                     | 提示下一步 `req split`       |
| N2 拆分子需求（向导）       | `/dev-workflow req split {域}/{父需求名}`                     | `.task/` 子目录                                | 子需求目录 + `metadata.md`（8 字段）                                                      | —                     | 提示下一步 `version create`  |
| N3 创建版本                 | `/dev-workflow version create {版本号}`                       | 全局 `metadata.md.versionBinding` 为空的子需求 | `docs/version/{版本号}` + 各 `metadata.md.versionBinding` 回填                            | version.status: DRAFT | 提示下一步 `version start`   |
| N4 增量加子需求（仅 DRAFT） | `/dev-workflow version add-sub {版本号} {子需求ID}`           | metadata.md.versionBinding / version.status    | 各 metadata.md + version.subRequirements[]                                                | version.status 不变   | 回显 subRequirements 总数    |
| N5 推进子需求下一阶段       | `/dev-workflow next {子需求ID}`                               | `metadata.md` + 各阶段产物                     | 各阶段产物 + `metadata.md.currentStage/currentState`                                      | 详见 §3.2             | 按 §1.2 流水线推进           |
| N6 确认人工门               | `/dev-workflow approve {子需求ID}`                            | 当前状态                                       | `design-consensus.md` / `plan.md` / 任务状态 / `metadata.md`                              | 视门类                | 详见 §3.3                    |
| N7 查看子需求或版本状态     | `/dev-workflow status [子需求ID\|版本号]`                     | metadata.md 或 version 文件                    | —                                                                                         | —                     | 输出进度展示；缺参时扫描全部 |
| N8 列父需求                 | `/dev-workflow req list`                                      | 所有 `.task/parent.md`                         | —                                                                                         | —                     | 输出清单 + 子需求数          |
| N9 列版本                   | `/dev-workflow version list`                                  | 所有版本文件                                   | —                                                                                         | —                     | 输出清单                     |
| N10 推进版本状态            | `/dev-workflow version start\|ready\|close\|archive {版本号}` | version.status + 子需求聚合                    | version.status + stageRecord                                                              | 见 §1.4 版本状态机    | 回显新状态                   |
| N11 返工                    | `/dev-workflow rework {子需求ID}`                             | 缺陷描述 + 依赖图 + version.status             | `.task/{子需求名}/rework/R{N}-{日期}.md` + `design-consensus.md`（设计级）+ `metadata.md` | 按层级回退            | 列出受影响子需求等用户确认   |
| N12 出交付清单              | `/dev-workflow summary {版本号}`                              | 版本的子需求产物 + 实际代码                    | 单域 / 跨域 change-manifest.md                                                            | —                     | 输出 DDL N / 队列 M / API K  |
| N13 多版本合并清单          | `/dev-workflow summary --merge {V1} {V2} ...`                 | 各版本 `change-manifest.md`                    | `docs/version/.merged/change-manifest-{Y}-{M}-{D}.md`                                     | —                     | 输出合并清单                 |
| N14 发现刷新                | `/dev-workflow discovery refresh`                             | manifest / CLAUDE.md / 架构文档                | 会话内存 DiscoveryContext                                                                 | —                     | 输出新发现字段               |
...
| S4 CR 扫描  | CODING                         | 单 `code-reviewer`               | diff + done + design-consensus + 架构文档 | `.task/{子需求名}/review/{模块}-{序号}.md`                               | CR_SCANNED → PENDING_CR_REVIEW      |
...
调用 `/dev-workflow discovery refresh` 的典型场景：
...
- 传子需求 ID：格式 `{域}/{父需求名}#{子需求名}`，按最后一个 `#` 拆分
...
> **v1 已删除（不存在的）路径**：`docs/discuss/.workflow-active`、`docs/discuss/{域}/{需求名}/.task/progress.md`、`docs/discuss/{域}/{需求名}/.task/milestones/{M}/`、`docs/discuss/{域}/{需求名}/.task/design-foundation.md`。
...
人工门：阶段 3 设计审核、阶段 4 plan 确认、阶段 4 CR 裁决（均为 `/dev-workflow approve {子需求ID}`）。
...
7. **设计/需求层缺陷走 rework**，不要硬塞进 CR 改写：CR 改写只解决任务内小问题；牵动设计或多任务用 `/dev-workflow rework {子需求ID}`。
...
11. **metadata.md 单源**：子需求状态唯一权威源是 `docs/discuss/{域}/{父需求名}/.task/{子需求名}/metadata.md`；version 文件不缓存子需求状态，只存 `subRequirements` 列表 + 阶段记录。
...
| 命令                              | 必带 / 可选               | 关键效果                                                   |
| --------------------------------- | ------------------------- | ---------------------------------------------------------- |
| `/dev-workflow req create`        | `{域}/{父需求名}`         | 创建父需求 + parent.md                                     |
| `/dev-workflow req show`          | `{域}/{父需求名}`         | 显示父需求详情 + 子需求清单                                |
| `/dev-workflow req list`          | （无）                    | 列出所有父需求                                             |
| `/dev-workflow req split`         | `{域}/{父需求名}`         | 交互式向导逐个创建子需求                                   |
| `/dev-workflow version create`    | `{版本号}`                | 4 步交互式建版本（DRAFT + 扫描 + 多选 + 回写）             |
| `/dev-workflow version show`      | `{版本号}`                | 显示版本详情 + 子需求状态聚合                              |
| `/dev-workflow version list`      | （无）                    | 列出所有版本                                               |
| `/dev-workflow version add-sub`   | `{版本号} {子需求ID}`     | DRAFT 增量加子需求                                         |
| `/dev-workflow version start`     | `{版本号}`                | DRAFT → IN_PROGRESS                                        |
| `/dev-workflow version ready`     | `{版本号}`                | IN_PROGRESS → READY                                        |
| `/dev-workflow version close`     | `{版本号}`                | READY → RELEASED                                           |
| `/dev-workflow version archive`   | `{版本号}`                | RELEASED → ARCHIVED（永久封存）                            |
| `/dev-workflow next`              | `{子需求ID}`              | 推进子需求到下一阶段                                       |
| `/dev-workflow approve`           | `{子需求ID}`              | 按当前状态分发：设计审核 / plan / CR 裁决                  |
| `/dev-workflow status`            | `[子需求ID\|版本号]`      | 显示进度                                                   |
| `/dev-workflow rework`            | `{子需求ID}`              | 缺陷返工（需用户选根因层级）                               |
| `/dev-workflow summary`           | `{版本号}`                | 产出 change-manifest.md（DDL/Job·MQ/API）                  |
| `/dev-workflow summary --merge`   | `{版本号A} {版本号B} ...` | 多版本合并清单（→ .merged/change-manifest-{Y}-{M}-{D}.md） |
| `/dev-workflow discovery refresh` | （无）                    | 清空发现缓存，重新解析必需字段                             |

> **总命令数**：19 启用（4 req + 8 version + 7 保留）+ 3 删除（use / split / start，v1 已硬切换废弃）。

子需求 ID 格式：`{域}/{父需求名}#{子需求名}`（按最后一个 `#` 拆分；ID 中如含 `#`，取最后一个为子需求分隔）。
...
/dev-workflow next order/订单取消优化#订单取消逻辑       # 推进任务2... 直到全部 DONE

/dev-workflow next order/订单取消优化#订单取消逻辑       # 阶段 5：verifier 全量 {test_cmd} + {lint_cmd} + 跨模块一致性
  → 通过 → COMPLETED

/dev-workflow version ready v1.0        # 所有子需求 COMPLETED 后推进版本
/dev-workflow version close v1.0        # 二次确认 → RELEASED
/dev-workflow summary v1.0              # 产出 change-manifest.md（DDL/Job·MQ/API）
```
...
/dev-workflow req split order/订单取消优化        # 创建订单取消逻辑
/dev-workflow req split payment/支付渠道重构      # 创建 alipay、wechat
...
/dev-workflow approve order/订单取消优化#订单取消逻辑   # plan 确认 → 编码 → CR ...
```
...
/dev-workflow next order/订单取消优化#订单取消逻辑    # 阶段 2 修订 design-consensus.md
/dev-workflow approve order/订单取消优化#订单取消逻辑 # 阶段 3 重审通过
/dev-workflow next order/订单取消优化#订单取消逻辑    # 复杂任务重出 plan → 重 code → 重 CR
```
...
/dev-workflow summary --merge v1.0 v1.1 v1.2
...
- [ ] 是否调用 `/dev-workflow approve {子需求ID}`（而非 `next`）
...
- [ ] 牵动设计或别的子需求 → 用 `/dev-workflow rework {子需求ID}`
...
- [ ] 下次 `/dev-workflow next {子需求ID}` 是否从该阶段继续
...
- [ ] 何时调 `/dev-workflow discovery refresh`：CLAUDE.md 改了、新增模块、架构文档重新生成、字段冲突错误后
...
> 本附录列出 `development/dev-workflow/` 全部源文件与本文档的**冲突点**。冲突来源可能是文档陈旧、迁移期内的中间状态、或源文件本身的内部不一致。**按用户要求：本文档不修改既有章节内容，仅追加新发现的冲突**，修复待用户决策。
...
| **references/discovery.md**（§1/§5/§6/§7/§10 共 11 段） | `docs/workflow/{模块}/`                                                                           |
...
**冲突点**：本文档大量出现 `docs/workflow/{模块}/`，所有 6 份源文件（含 references 全部子文件）一致使用 `docs/workflow/{模块}/`。
...
| 来源                                                            | 单域版本路径                                                               | 跨域版本路径                                                                |
| --------------------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **references/summary.md §4 / §5（document-specialist prompt）** | `docs/discuss/{域}/{父需求名}/.task/.versions/{版本号}/change-manifest.md` | `docs/version/{版本号}/change-manifest.md`（子目录 + `change-manifest.md`） |
...
- **stage-4-dev.md §★ CR 子 agent 返回后**：「零问题（PASSED）也要输出'无需裁决，执行 approve'」「自检：如果你的输出中没有包含 `/dev-workflow approve` 这个字符串，说明你漏了第 3 步，立刻补上」
...
**修复方向**：§11.3 PENDING_CR_REVIEW 自检清单补一项「零问题 → 也要输出'执行 /dev-workflow approve'，不许静默」。
...
- **扫描源**：`development/dev-workflow/` 全部 12 个文件（SKILL.md / README.md / MIGRATION.md + references/{commands, discovery, templates, stage-2-design, stage-3-review, stage-4-dev, stage-5-accept, rework, summary}.md）
