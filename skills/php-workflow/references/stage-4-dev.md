...
- **倾向并行**：存在 ≥2 个**互不依赖**、且**目录不重叠**（`modules/{service-name}/` 不重叠，无文件冲突）的子需求，并行能明显省时。并行时用 `team {N}:executor`，每个 executor 工作范围限定在自己的子需求目录与对应模块。
- **倾向串行**：子需求之间有依赖链、共享同一模块/文件、子需求很少或很小、或并行收益不明显。串行时用单个 `executor`，一次一个。
- **混合**：常见做法是按"波次"组织——同一波内**互不依赖**的子需求并行编码（满足"互不依赖 + `modules/{service-name}/` 不重叠"两个条件），波与波之间按依赖串行；有依赖的子需求把前序产出（接口/Event 契约）注入 prompt。
...
```
# 并行（同一波内互不依赖、modules/{service-name}/ 不重叠的子需求）
/oh-my-claudecode:team {N}:executor "
本波任务（互不依赖，按子需求隔离、modules/{service-name}/ 互不重叠）：
{对本波每个子需求生成一行}
- 子需求 [{service-name}/{父需求名}#{子需求名}] executor: 工作范围限定在 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/ 与 modules/{service-name}/
  任务: {从 dev-tasks.md 提取}
  {复杂任务：注入已确认的 plan 路径 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/plans/{任务序号}-{service-name}.md，严格对照实现；简单任务：对照 design-consensus 实现要点}
  {若依赖前序波次，注入 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/done/{前序模块}.md 中的接口/Event 契约}
  DoD: 写代码 + 写单测 + vendor/bin/phpunit modules/{service-name}/tests 绿 + 对改动文件执行 php -l 语法校验通过
  完成后：① 把本任务在 dev-tasks.md 中已完成的子项 [ ] 勾成 [x]；② 写入 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/done/{service-name}.md（改动摘要、对外接口/Event 契约、phpunit 与 php -l 结果）
...
```
executor "
按已确认的审查裁决修复子需求 [{service-name}/{父需求名}#{子需求名} · {任务标题}]，工作范围 modules/{模块名}/ 与 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/：
...
- **编码并行/串行由 Claude 判断**：并行仅限两个子需求同时进入 CODING（满足"互不依赖 + 目录不重叠（`modules/{service-name}/` 不重叠）"）；有依赖的按波次串行，前序产出（接口/Event 契约）注入后续 prompt
