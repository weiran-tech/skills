...
{对每个受影响的模块生成一行}
- modules/{模块名} → 读代码 + 读 docs/workflow/{模块名}/ → 产出 docs/discuss/{域}/{父需求名}/.task/{子需求名}/analysis/{模块名}.md
...
- docs/discuss/{域}/{父需求名}/.task/{子需求名}/dev-tasks.md（含开发任务、依赖顺序、模块路径 modules/{模块名}；并对每个任务标注复杂度 简单|复杂，复杂任务在阶段 4 编码前需出 plan）
