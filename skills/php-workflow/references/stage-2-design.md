...
> 执行阶段 2 前读本文件。将模块分析和汇总设计自动串联为一个阶段。产出物根路径见 templates.md。
...
按照讨论文档 docs/discuss/{需求ID}/discussion.md 的影响分析，分别分析各受影响模块（同一仓库内）：
如 docs/discuss/{需求ID}/docs/ 下有参考文档，先读取作为需求背景。
{对每个受影响的模块生成一行}
- modules/{模块名} → 读代码 + 读 docs/{模块名}/ → 产出 docs/discuss/{需求ID}/.task/analysis/{模块名}.md
...
```
ralph: "读取 docs/discuss/{需求ID}/.task/analysis/ 下全部分析文档，
结合 docs/discuss/{需求ID}/discussion.md 讨论文档，
如 docs/discuss/{需求ID}/docs/ 下有参考文档，一并参照。
参照 docs/cross-module.md 校验跨模块事件链（Event/Listener）的完整性。

生成：
- docs/discuss/{需求ID}/.task/design-consensus.md —— 共识/契约层，**必含清单**（缺项视为未完成）：
...
- docs/discuss/{需求ID}/.task/dev-tasks.md（含开发任务、依赖顺序、模块路径 modules/{模块名}；并对每个任务标注复杂度 简单|复杂，复杂任务在阶段 4 编码前需出 plan）
