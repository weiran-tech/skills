...
> 处理 `/php-workflow summary [版本号]` 时读本文件。用途：对已 READY 或 RELEASED 的版本，聚合其下所有子需求的 DDL 变更、新增队列、API 接口清单，输出一份对接清单分发给 DBA / 运维 / 前端（AC9）。粒度对齐 spec C10：`rework` 按子需求，`summary` 按版本聚合。
...
- 错误信息三段式：`[字段 command.args] 缺少版本号。建议：执行 /php-workflow version list 查可用版本；或直接传 /php-workflow summary {版本号}`
...
|----|------------------------------------------|
| **DDL 变更** | `dev-tasks.md` 中带「迁移」标签的任务条目 / `done/` 已归档任务里带「migration」标志的文件 / `acceptance.md` 的「DB 变更」段 / 实际代码 `modules/*/resources/migrations/*` 跨任务汇总 |
...
主 Agent 收到 `/php-workflow summary {版本号}` 后：

### 步骤 1：解析与校验

```
read {项目根}/docs/version/{版本号}
  ├─ 不存在 → 报错 [字段 version.file] {版本号} 不存在。建议：执行 /php-workflow version list 查可用版本
...
**所有相关版本 RELEASED 后**，用户可主动调用 `/php-workflow summary --merge {版本号A} {版本号B} ...` 出需求级合并清单：把各版本 change-manifest.md 的三块拼起来，每行带 `(来源: {版本号} / {子需求ID})` 标注。此命令不自动触发，仅在用户显式传 `--merge` 时启用。
...
| 场景 | 错误信息（三段式） | 建议 |
|------|-------------------|------|
| 版本文件不存在 | `[字段 version.file] {版本号} 不存在` | 执行 `/php-workflow version list` 查可用版本 |
...
| 缺参（无版本号） | `[字段 command.args] 缺少版本号` | 执行 `/php-workflow version list` 查可用版本；或直接传 `/php-workflow summary {版本号}` |
...
| 产物 | 命令 | 性质 | 受众 |
|------|------|------|------|
| change-manifest.md | `/php-workflow summary {版本号}` | 事实清单（DDL/Job/API） | DBA + 前端 + 运维 |
...
`/php-workflow summary --merge {版本号A} {版本号B} ...` — 仅在用户显式传 `--merge` 时启用。
