...
- 多里程碑提示用 `/php-workflow use #{下一个里程碑}` 推进或全部完成
- 提示用户可执行 `/php-workflow summary` 产出交付清单
...
- **明确提示**：
  ...
  - 如果问题属于设计/需求层缺陷，建议使用 /php-workflow rework

  裁决完成后执行 /php-workflow approve 推进修复
  ```
- **停在 PENDING_ACCEPT_REVIEW 等待用户**，绝不自动派 executor 修复

**C. 验收发现设计错 / 多任务受牵连** → 在问题清单中标注，提示用户使用 `/php-workflow rework`（设计级，见 rework.md）
...
- 全部裁决完成后，用户执行 `/php-workflow approve`
- **⚠️ 只有用户显式输入 `/php-workflow approve` 才算裁决锁定。** 用户口头说裁决结果是在提供裁决信息，主 Agent 应回填到 acceptance.md，但不得自动推进到修复

### ★ approve 后修复（ACCEPT_FIXING）

`/php-workflow approve` 确认后：
