...
> 执行任意 `/php-workflow` 子命令前读本文件。`rework` 的详细流程见 rework.md；各阶段执行细节见 stage-*.md。
...
7. 提示用户可用 `/php-workflow next` 进入分析与设计阶段（无需再带 ID）
...
5. 提示用户用 `/php-workflow next` 开始该任务编码（对照已确认 plan）
...
7. 提示用户用 `/php-workflow next` 继续下一个任务（或若全部 DONE，进入阶段 5）
...
5. 如果当前是 PENDING_DESIGN_REVIEW / PENDING_PLAN_REVIEW / PENDING_CR_REVIEW / PENDING_ACCEPT_REVIEW，提示用户先 `/php-workflow approve`（不重复执行已停住的人工门）
...
## `/php-workflow followup {已完成需求ID} [新需求名]`
...
1. **解析父需求 ID**（支持模糊匹配）。校验其 progress.md 状态为 **COMPLETED**；非 COMPLETED 则拒绝并提示：
   - 进行中的需求 → 用 `/php-workflow next` 继续推进
   - 有设计/实现缺陷 → 用 `/php-workflow rework`
...
7. 提示用户讨论完成后 `/php-workflow next` 进入分析与设计
...
> **与 `/php-workflow start` 的区别**：`start` 从零开始，无父需求上下文；`followup` 自动继承设计产出物、讨论聚焦增量、progress.md 记录关联关系。后续阶段 2 分析时 analyst 会同时参考父需求设计和新讨论结论。
