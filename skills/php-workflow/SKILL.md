...
面向伪多模块单体项目：代码同仓 `modules/{service-name}/`，跨模块走 Events/Listeners + Service，审查按模块路径过滤 diff（`git diff <默认分支>...HEAD -- modules/{service-name}`），架构上下文来自 `docs/workflow/{service-name}/`（php-analyzer 产出）。测试/校验等具体命令默认按 PHP 工具链（phpunit / php -l），可按项目配置替换。
...
```
/php-workflow req create {service-name}/{父需求名}                    # 创建父需求（单子需求场景）
/php-workflow req show {子需求ID}                          # 查看子需求详情（metadata.md + 阶段产物）
/php-workflow req list                                    # 列出所有子需求（含状态/版本绑定）
/php-workflow req split {service-name}/{父需求名}                    # 把大需求拆成多个子需求（交互式向导）
```

### version 子命令族（版本管理，9 个）

```
/php-workflow version create {版本号}                      # 创建版本（4 步流程：建空 DRAFT → 扫描未绑定 → 交互多选 → 回写 metadata.md.versionBinding）
/php-workflow version show {版本号}                        # 查看版本详情（含聚合子需求状态）
/php-workflow version list                                # 列出所有版本（含状态/子需求数）
/php-workflow version add-sub {版本号} {子需求ID}          # 向版本添加子需求（仅 DRAFT 状态可执行）
/php-workflow version start {版本号}                       # DRAFT → IN_PROGRESS（前置：至少 1 个子需求）
/php-workflow version ready {版本号}                       # IN_PROGRESS → READY（前置：所有子需求 COMPLETED）
/php-workflow version close {版本号}                       # READY → RELEASED（人工确认发布）
/php-workflow version archive {版本号}                     # RELEASED → ARCHIVED（永久封存，禁止再修改）
```

### 流程推进（5 个保留）

```
/php-workflow next {子需求ID}                             # 执行下一阶段 / 下一个任务
/php-workflow approve {子需求ID}                          # 确认当前人工门（阶段3设计 / 阶段4 plan / 阶段4 CR）
/php-workflow status {子需求ID}                           # 查看进度（省略=列出所有进行中子需求）
/php-workflow rework {子需求ID}                           # 设计/实现缺陷返工（按根因层级回退 + 依赖级联重做）
/php-workflow summary {版本号}                            # 产出交付清单（DDL / Job·MQ / API）给 DBA 与前端（按版本聚合）
```
...
- **架构文档** `docs/workflow/{service-name}/`（overview/business/contracts/flows）+ `docs/workflow/cross-module.md`，由 `php-analyzer` 产出；缺失则先提示跑 php-analyzer
- **验收基线（强制）**：`vendor/bin/phpunit modules/{service-name}/tests` 单测绿 + 改动文件 `php -l` 语法校验通过
...
8. **子需求默认单个**：只有大需求才 `req split`；跨模块交互一律通过 Service/Events，禁止直接访问他模块 Model/Repository。
9. **无活动指针**：删除 `docs/discuss/.workflow-active`；所有命令必须显式传子需求 ID 或版本号，无任何隐式解析或粘性默认值。`next/approve/status/rework/summary` 缺参 → 报错 + 列出可选项。
...
| 执行任意 `/php-workflow` 子命令的详细逻辑（含 req/version/next/approve 等）   | `references/commands.md`          |
...
| 执行 `/php-workflow summary` 产出交付清单（按版本聚合 DDL/Job·MQ/API）        | `references/summary.md`           |
