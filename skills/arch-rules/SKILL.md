...
| `.claude/rules/cross-service-guide.md` | 混合 | 模板 + docs/扫描 | 跨服务改动清单 + 引用，不重复 MQ/RPC 正文（globs 动态生成） |
...
mq-conventions.md 和 cross-service-guide.md 的 frontmatter globs 必须从第一步获取的项目信息中提取实际路径，不能写死。例如：
