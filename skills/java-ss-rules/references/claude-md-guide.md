...
| 顺序 | 文档 | 什么时候看 |
|------|------|-----------|
| 1 | [docs/workflow/{service-name}/overview.md](docs/workflow/{service-name}/overview.md) | 初次进入：服务职责、模块结构、技术栈、上下游依赖 |
| 2 | [docs/workflow/{service-name}/business.md](docs/workflow/{service-name}/business.md) | 实现业务逻辑前：状态机、业务规则 |
| 3 | [docs/workflow/{service-name}/contracts.md](docs/workflow/{service-name}/contracts.md) | 改 MQ/RPC 前：事件契约全表、接口依赖 |
| 4 | [docs/workflow/{service-name}/flows.md](docs/workflow/{service-name}/flows.md) | 评估影响范围：执行流程、事件级联链路 |

{只列出 docs/workflow/ 下实际存在的文件}
...
**关键原则**：
- Tech Stack 和 Modules 由 `docs/workflow/{service-name}/overview.md` 持有，CLAUDE.md 不重复
