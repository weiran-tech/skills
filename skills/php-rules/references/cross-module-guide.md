...
列出 `docs/workflow/` 目录下实际存在的文件，说明什么时候需要看：
...
| 文档 | 什么时候看 |
|------|-----------|
| `docs/workflow/{service-name}/contracts.md` | 新增/修改 Event 或跨模块调用前（避免事件冲突、参数不兼容） |
| `docs/workflow/{service-name}/flows.md` | 评估改动的下游影响范围时 |
| `docs/workflow/{service-name}/business.md` | 实现业务逻辑前（理解状态机和业务规则） |
| `docs/workflow/{service-name}/overview.md` | 初次进入某个模块时 |
| `docs/workflow/cross-module.md` | 改动涉及多个模块时（理解模块间依赖关系） |
```
