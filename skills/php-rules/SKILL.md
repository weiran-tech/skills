...
| `.claude/rules/http-conventions.md`   | 混合   | 模板 + 扫描  | HTTP 接口、后台 Controller、模板与分页的开发规范      |
...
docs/workflow/{service-name}/overview.md   → 事实（模块是什么、目录结构、依赖）     ← php-analyzer 生成
docs/workflow/{service-name}/business.md   → 规则（为什么这么处理）               ← php-analyzer 生成
docs/workflow/{service-name}/contracts.md  → 契约（路由、事件、Job）              ← php-analyzer 生成
docs/workflow/{service-name}/flows.md      → 链路（怎么流转）                     ← php-analyzer 生成
architecture.md          → 通用编码规则（分层约束、设计原则、编码标准）
module-map.md            → 速查（模块在哪、管什么）
event-conventions.md     → Event/Listener/Job 开发模板
http-conventions.md     → HTTP 接口、后台 Controller、模板与分页的开发规范
...
检查 `docs/workflow/` 目录是否包含各模块的文档。如果存在，直接读取：

- `docs/workflow/{service-name}/overview.md` → 模块职责、目录结构、技术栈、跨模块依赖
- `docs/workflow/{service-name}/contracts.md` → 事件契约、路由清单、Job 列表、跨模块调用
- `docs/workflow/cross-module.md` → 模块间依赖矩阵
...
3. **`.claude/rules/http-conventions.md`** ← 读取 `references/http-conventions-guide.md`
4. **`.claude/rules/cross-module.md`** ← 读取 `references/cross-module-guide.md`

### 第三步补充：globs 动态生成

event-conventions.md 和 http-conventions.md 和 cross-module-guide.md 的 frontmatter globs 必须从第一步扫描结果中提取实际路径：
