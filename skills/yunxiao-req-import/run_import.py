    ...
    for i, row in enumerate(rows, 1):
        ...
        try:
            ...
            result = subprocess.run(
                ["npx", "@anthropic-ai/mcp-cli", "call", "mcp__yunxiao__create_work_item",
                 "--organizationId", ORG_ID,
                 "--spaceId", SPACE_ID,
                 "--subject", subject,
                 "--workitemTypeId", type_id,
                 "--assignedTo", assignee_id,
                 "--customFieldValues", json.dumps(custom_fields),
            ...
            if result.returncode == 0:
                ...
                failed += 1
                print(f"❌ [{i}/{len(rows)}] {subject[:30]}... 错误: {result.stderr[:100]}")

        except Exception as e:
            failed += 1
            print(f"❌ [{i}/{len(rows)}] {subject[:30]}... 错误: {str(e)[:100]}")
