...
状态机驱动的开发流程 skill。把"需求讨论 → 分析设计 → 开发 → 审查 → 验收"串成一个统一入口，用 `progress.md` 记录状态，每次 `/devops-workflow next` 自动决定下一步调度哪个 agent、传什么参数。

面向单体项目：代码同仓，审查按任务工作范围过滤 diff，架构上下文取自项目文档目录（arch-analyzer 产出）。测试/校验命令由项目 `.claude/rules/` 中的规则定义，workflow 不硬编码具体命令和目录结构。
...
```
  阶段1 需求讨论        /devops-workflow start  → devops-discuss 多轮对话
     │ (产出 docs/discuss/{域}/{需求名}/discussion.md)
     ▼
  阶段2 分析与设计      /devops-workflow next   → analyst 逐模块分析 + ralph 汇总设计
     │ (产出 analysis/、design-consensus.md[必含清单]、dev-tasks.md)
     ▼
 ┌──────────────────────────────────────────┐
 │ 阶段3 设计审核门  ★人工★  PENDING_DESIGN_REVIEW │  ← 清单式审核，缺项打回阶段2
 │   /devops-workflow approve                        │
 └──────────────────────────────────────────┘
     ▼
  阶段4 开发与逐任务审查  /devops-workflow next  （见下方"任务级闭环"）
     │
     ▼
  阶段5 收尾验收        /devops-workflow next   → verifier 全量回归 + 一致性把关（只读，不改代码）
 ...
 └──────────────────────────────────────────┘
  ...
  ACCEPT_FIXING → executor 修复已采纳项 → 重跑收尾验收
     ▼
  COMPLETED ──→ /devops-workflow summary  → 交付清单 change-manifest.md（DDL / Job·MQ / API，给 DBA 与前端）

  ※ 阶段4/5 若发现设计/需求层缺陷 → /devops-workflow rework 回退重做
```
...
TODO
 ...
 ▼ CODING
 │ ② DoD：模块级单测通过 + 语法/编译检查通过（按项目 .claude/rules/）
 ...
 │ ⑤ 改写 (executor，只改已采纳项)
 ▼ REWRITING
 │ ⑥ 复验 + 进度回写（强制）：模块级单测通过 + 语法/编译检查通过 → 回写 progress.md + dev-tasks.md
 ▼ VERIFYING → DONE
```
...
- **大需求**：用 `/devops-workflow split` 按交付切片拆成多个里程碑（如按渠道/子系统/上线批次拆分）。拆分后每个里程碑独立跑阶段 2→5，公共设计骨架（`design-foundation.md`）须先于各里程碑定稿；用 `#里程碑` 选择符分别推进。
...
| 命令 | 作用 |
|------|------|
| `/devops-workflow use [需求ID][#里程碑]` | **设定活动上下文（粘性）**，之后裸命令默认作用于它 |
| `/devops-workflow start {需求名}` | 创建需求，进入阶段1讨论（自动设为活动）|
| `/devops-workflow next [需求ID][#里程碑]` | 推进到下一阶段 / 下一个任务（省略=活动上下文）|
| `/devops-workflow approve [需求ID][#里程碑]` | 确认当前人工门（设计审核 / plan / CR 裁决 / 验收问题裁决，按状态自动分发）|
| `/devops-workflow status [需求ID][#里程碑]` | 查看进度（省略=活动上下文+高亮；无活动则全部）|
| `/devops-workflow list` | 列出未完成的需求 |
| `/devops-workflow split [需求ID] {里程碑列表}` | 大需求拆里程碑 |
| `/devops-workflow followup {已完成需求ID} [新需求名]` | 基于已完成需求发起新需求（继承设计上下文） |
| `/devops-workflow rework [需求ID][#里程碑]` | 设计/实现缺陷返工 |
| `/devops-workflow summary [需求ID][#里程碑]` | 产出交付清单（DDL / Job·MQ / API）给 DBA 与前端，验收通过后执行 |

- **需求 ID** = `{域}/{需求名}`（如 `order/订单取消优化`），与 `docs/discuss/` 目录一致
- **`#里程碑`** 仅多里程碑需求需要（如 `payment/支付渠道重构#alipay`）
- **活动上下文（推荐）**：`/devops-workflow use` 选一次当前在搞的需求/里程碑（存于 `docs/discuss/.workflow-active`），之后 `next/approve/status/rework` 省略参数即默认指向它，不必每步重复敲长 ID
...
/devops-workflow next       # 推进任务2…… 直到全部 DONE

# 5. 收尾验收（verifier 全量单测 + 语法/编译检查 + 跨模块一致性）
/devops-workflow next
#   → 通过则 COMPLETED
...
```bash
# 需求已讨论完，按交付切片拆里程碑
/devops-workflow split payment/支付渠道重构 alipay,wechat
#   → 确认 alipay/wechat 划分 + 抽出 design-foundation.md（公共骨架，先定稿）

# 选一次活动上下文，之后裸命令默认作用于它（不必每步敲长 ID）
/devops-workflow use payment/支付渠道重构#alipay     # 也可模糊：use payment#alipay

/devops-workflow next        # 阶段2 分析设计 → PENDING_DESIGN_REVIEW
/devops-workflow approve     # 设计审核通过

/devops-workflow next        # 阶段4：某任务判为【复杂】→ architect 出 plan，停在 PENDING_PLAN_REVIEW
/devops-workflow approve     # plan 确认 → 编码 → CR 扫描 → PENDING_CR_REVIEW …

# 切到另一个里程碑：只敲 #
/devops-workflow use #wechat
/devops-workflow next
...
/devops-workflow followup order/订单取消优化 取消原因统计
#   → 自动继承父需求设计文档到 docs/parent/
#   → 进入阶段 1 讨论，聚焦增量变更
#   → 讨论完成后 /devops-workflow next 走标准 5 阶段流程
```
...
/devops-workflow rework payment/支付渠道重构#alipay
#   → 确认根因层级 = 设计级
...
# 修订设计 → 重审 → 受影响任务级联重跑
/devops-workflow next payment/支付渠道重构#alipay     # 阶段2 修订 design-consensus
/devops-workflow approve payment/支付渠道重构#alipay  # 阶段3 重审通过
/devops-workflow next payment/支付渠道重构#alipay     # 复杂任务重出 plan → 重 code → 重 CR
```
...
docs/discuss/
  ...
  .task/
    ...
    change-manifest.md           # /devops-workflow summary 交付清单（DDL/Job·MQ/API，给 DBA 与前端）
...
- **测试/校验**：验收基线 = 按项目 `.claude/rules/` 中定义的测试命令执行模块级单测通过 + 按项目规则执行语法/编译检查通过。静态分析是否纳入验收由项目规则决定
- **单仓库单分支**：共用一条 feature 分支，diff 用 `git diff <默认分支>...HEAD -- {任务工作范围目录}` 按范围隔离；**无独立分支/PR**
