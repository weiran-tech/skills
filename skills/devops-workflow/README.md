...
|------|---------|------|--------------|
| `/devops-workflow req create` | `{域}/{父需求名}` | 创建父需求目录骨架 | 替代 `start` |
| `/devops-workflow req show` | `{域}/{父需求名}` | 显示父需求详情 + 子需求清单 | — |
| `/devops-workflow req list` | （无） | 列出所有父需求 | — |
| `/devops-workflow req split` | `{域}/{父需求名}` | 交互式向导逐个创建子需求 | 替代 `split` |
...
| `/devops-workflow start {需求名}` | ❌ 已删除 | **`/devops-workflow req create {域}/{父需求名}`** —— 创建父需求后，用 `req split` 进入子需求创建向导 |
