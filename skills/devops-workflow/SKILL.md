...
三道人工门（均由 `/devops-workflow approve` 按状态分发）：**阶段3 设计审核** / **阶段4 复杂任务 plan** / **阶段4 CR 问题裁决**。
...
```
/devops-workflow req create {service-name}/{父需求名}                    # 创建父需求（单子需求场景）
/devops-workflow req show {子需求ID}                          # 查看子需求详情（metadata.md + 阶段产物）
/devops-workflow req list                                    # 列出所有子需求（含状态/版本绑定）
/devops-workflow req split {service-name}/{父需求名}                    # 把大需求拆成多个子需求（交互式向导）
```

### version 子命令族（版本管理，8 个）

```
/devops-workflow version create {版本号}                      # 创建版本（4 步流程：建空 DRAFT → 扫描未绑定 → 交互多选 → 回写 metadata.md.versionBinding）
/devops-workflow version show {版本号}                        # 查看版本详情（含聚合子需求状态）
/devops-workflow version list                                # 列出所有版本（含状态/子需求数）
/devops-workflow version add-sub {版本号} {子需求ID}          # 向版本添加子需求（仅 DRAFT 状态可执行）
/devops-workflow version start {版本号}                       # DRAFT → IN_PROGRESS（前置：至少 1 个子需求）
/devops-workflow version ready {版本号}                       # IN_PROGRESS → READY（前置：所有子需求 COMPLETED）
/devops-workflow version close {版本号}                       # READY → RELEASED（人工确认发布）
/devops-workflow version archive {版本号}                     # RELEASED → ARCHIVED（永久封存，禁止再修改）
```

### 流程推进（7 个保留）

```
/devops-workflow next {子需求ID}                             # 执行下一阶段 / 下一个任务
/devops-workflow approve {子需求ID}                          # 确认当前人工门（阶段3设计 / 阶段4 plan / 阶段4 CR）
/devops-workflow status {子需求ID}                           # 查看进度（省略=列出所有进行中子需求）
/devops-workflow rework {子需求ID}                           # 设计/实现缺陷返工（按根因层级回退 + 依赖级联重做）
/devops-workflow summary {版本号}                            # 产出交付清单（DDL / Job·MQ / API）给 DBA 与前端（按版本聚合）
/devops-workflow summary --merge {V1} {V2} ...               # 多版本合并清单（写 docs/version/.merged/change-manifest-{Y}-{M}-{D}.md）
/devops-workflow discovery refresh                            # 清空 §7 缓存，重跑 §1 发现（CLAUDE.md 修改后 / 新增模块后触发）
```
...
- `start {需求名}` → **改为**：`req create {service-name}/{父需求名}` + `req split {service-name}/{父需求名}`（向导式创建子需求）
- `split {ID} {里程碑列表}` → **改为**：`req split {service-name}/{父需求名}`（交互式向导逐个询问子需求 名/描述/模块影响）

- **子需求 ID** = `{service-name}/{父需求名}#{子需求名}`（如 `payment/支付渠道重构#alipay`），与目录 `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/` 一致
- **版本号**：任意非空字符串（如 `v1.0` / `v1.0-rc1` / `2024Q1`），仅做去重检查，不做 SemVer 校验
- 命令的详细处理逻辑见 `references/commands.md`

## 项目约定

- **单体仓库**：默认已在项目仓库根目录。子需求产出物存于 `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/`（目录结构见 `references/templates.md`）
- **版本文件存于** `docs/version/{版本号}`（全局资源，跨父需求、跨业务域）
- **模块路径** `{module_root_glob}/`；模块清单从项目 `manifest file` + `CLAUDE.md` 运行时声明获取
- **架构文档** `docs/workflow/{service-name}/`（overview/business/contracts/flows）+ `docs/workflow/cross-module.md`，由 `{discovery_cmd}` 产出；缺失则先提示跑 `{discovery_cmd}`
- **验收基线（强制）**：`{test_cmd} {module_root_glob}/{service-name}/tests` 单测绿 + 改动文件 `{lint_cmd}` 语法校验通过
...
2. **CR 问题人工门**：code-reviewer 只产出问题清单，**绝不直接改代码**；扫出的问题必须经人工逐条裁决（ACCEPTED/REJECTED/MODIFIED）并 `/devops-workflow approve` 后，才由另一个 executor 对已采纳项改写。零问题（PASSED）也要输出"无需裁决，执行 approve"，不许静默卡住。
...
11. **metadata.md 单源**：子需求状态唯一权威源是 `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/metadata.md`；`docs/version/{版本号}` 不缓存子需求状态，只存子需求 ID 列表 + 聚合视图。子需求 ↔ 版本 一对一绑定（`metadata.md.versionBinding` 单一字符串）。
...
| 执行任意 `/devops-workflow` 子命令的详细逻辑（含 req/version/next/approve/summary/summary --merge/discovery refresh 等）   | `references/commands.md`          |
...
| 执行 `/devops-workflow summary` 产出交付清单（按版本聚合 DDL/Job·MQ/API）        | `references/summary.md`           |
