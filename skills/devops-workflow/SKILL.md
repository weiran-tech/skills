...
auto_summary=true       # 验收通过 COMPLETED 后自动产出交付清单
```
...
1. **Preflight 检查（仅 `start`）**：执行 `start` 时先读 `references/preflight.md` 逐项检查，blocking 项未通过则终止。其他子命令不执行 preflight
...
- **前置检查（强制）**：见 `references/preflight.md`，所有检查项在命令执行前自动逐项验证
...
| 当你要做 | 先读 |
|---------|------|
| **任何命令执行前（最先）** | `references/preflight.md` |
