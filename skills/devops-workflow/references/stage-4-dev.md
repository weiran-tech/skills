...
- **倾向并行**：存在 ≥2 个**互不依赖**、且**目录不重叠**（`{module_root_glob}/{service-name}/` 不重叠，无文件冲突）的子需求，并行能明显省时。并行时用 `team {N}:executor`，每个 executor 工作范围限定在自己的子需求目录与对应模块。
- **倾向串行**：子需求之间有依赖链、共享同一模块/文件、子需求很少或很小、或并行收益不明显。串行时用单个 `executor`，一次一个。
- **混合**：常见做法是按"波次"组织——同一波内**互不依赖**的子需求并行编码（满足"互不依赖 + `{module_root_glob}/{service-name}/` 不重叠"两个条件），波与波之间按依赖串行；有依赖的子需求把前序产出（接口/{contract_type} 契约）注入 prompt。
...
TODO
 ...
 ▼ CODING
 │ ② DoD 自检：{test_cmd} 绿 + {lint_cmd} 语法校验通过
...
```
architect "
为子需求 [{service-name}/{父需求名}#{子需求名} · {任务标题}] 产出任务级详细设计（LLD），写入 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/plans/{任务序号}-{模块名}.md。
依据：design-consensus.md（契约/边界/决策）、docs/workflow/{模块名}/、相关现有代码。
必含：
1. 类与方法签名（精确到参数/返回类型）
2. 数据模型字段 + 类型（如有迁移，列字段/索引/默认值）
3. 调用时序 / 与现有代码的集成点（在哪些入口/服务/{contract_type} 接入）
4. 错误与边界处理
5. 要写的测试用例清单（用例名 + 覆盖点）
6. 迁移/回滚（如涉及）
只做设计，不写业务代码。
执行约束见 SKILL.md 不变量 12：所有 Bash 命令必须可静态分析，禁止 for/while/if/case/here-doc/嵌套 $()；产出物只落项目内 .task/ 或 docs/version/，禁止 home 或仓库外。
如项目配置了 {static_analysis_cmd}（静态分析器），可作为黑盒补充检查契约一致性。"
```
### ★ architect/planner 返回后，主 Agent 必须立刻执行以下动作（缺一不可，禁止静默结束本轮）
1. **确认文件已落盘**：检查 `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/plans/{任务序号}-{模块名}.md` 是否存在且有内容。如果 architect 只在对话中输出了 plan 但没写文件，主 Agent **必须手动将 plan 内容写入该文件**
2. **回写** metadata.md：把该任务状态置 `PLANNING` → `PENDING_PLAN_REVIEW`
3. **向用户输出 plan 摘要**：列出关键设计点（类/方法签名、数据模型、调用时序、测试用例清单）
4. **明确告知**：
   ```
   plan 已保存到 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/plans/{文件名}
...
```
# 并行（同一波内互不依赖、{module_root_glob}/{service-name}/ 不重叠的子需求）
/oh-my-claudecode:team {N}:executor "
本波任务（互不依赖，按子需求隔离、{module_root_glob}/{service-name}/ 互不重叠）：
{对本波每个子需求生成一行}
- 子需求 [{service-name}/{父需求名}#{子需求名}] executor: 工作范围限定在 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/ 与 {module_root_glob}/{service-name}/
  任务: {从 dev-tasks.md 提取}
  {复杂任务：注入已确认的 plan 路径 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/plans/{任务序号}-{模块名}.md，严格对照实现；简单任务：对照 design-consensus 实现要点}
  {若依赖前序波次，注入 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/done/{前序模块}.md 中的接口/{contract_type} 契约}
  DoD: 写代码 + 写单测 + {test_cmd} {module_root_glob}/{service-name}/tests 绿 + 对改动文件执行 {lint_cmd} 语法校验通过
  完成后：① 把本任务在 dev-tasks.md 中已完成的子项 [ ] 勾成 [x]；② 写入 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/done/{模块名}.md（改动摘要、对外接口/{contract_type} 契约、{test_cmd} 与 {lint_cmd} 结果）
...
1. **读取** executor 写出的完成报告 `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/done/{模块名}.md`（以文件为准）
2. **回写** metadata.md：把该子需求任务状态置 `CODING`（如尚未置）
3. **向用户输出**：
   - 改动摘要（改了哪些文件/类、新增了什么）
   - {test_cmd} / {lint_cmd} 执行结果
   - 发现的问题（如旧文件未清理等）
...
```
code-reviewer "
审查子需求 [{service-name}/{父需求名}#{子需求名} · {任务标题}] 在当前分支上的实际代码变更：
1. 执行 git diff <默认分支>...HEAD -- {module_root_glob}/{模块名} 获取该模块变更内容
2. 读取 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/done/{模块名}.md 与 .task/design-consensus.md 了解任务意图与契约
3. 读取 docs/workflow/{模块名}/ 了解该模块既有契约与业务规则
4. 基于实际变更审查：与设计/任务一致性、逻辑正确性、单测覆盖、跨模块契约一致性、是否符合 CLAUDE.md 与 .claude/rules/
5. 把问题整理成**编号清单**写入 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/review/{模块名}-{任务序号}.md，每条含：编号 / 严重度(critical|major|minor) / 文件:行 / 问题描述 / 建议改法 / 裁决:PENDING
...
1. **读取** code-reviewer 写出的报告 `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/review/{模块名}-{任务序号}.md`（子 agent 的返回文本不算数，以报告文件为准）
...
- 裁决方式二选一：直接在 `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/review/{模块名}-{任务序号}.md` 的『人工裁决』区标注，或口头告知由主 Agent 回填
...
```
executor "
按已确认的审查裁决修复子需求 [{service-name}/{父需求名}#{子需求名} · {任务标题}]，工作范围 {module_root_glob}/{模块名}/ 与 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/：
只处理 docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/review/{模块名}-{任务序号}.md 中裁决为 ACCEPTED / MODIFIED 的问题（REJECTED 的不动）。
逐条修复后在该问题下标注『已修复』并简述改动。
修复后重跑 {test_cmd} {module_root_glob}/{service-name}/tests 与对改动文件 {lint_cmd} 确认通过，并把 dev-tasks.md 中剩余已完成子项 [ ] 勾成 [x]。
遵守 CLAUDE.md 架构规则。
执行约束见 SKILL.md 不变量 12：所有 Bash 命令必须可静态分析，禁止 for/while/if/case/here-doc/嵌套 $()。"
```

### ⑥ 复验与进度回写（强制，做完才算 DONE）
确认已采纳问题均已修复、`{test_cmd}` 绿 + `{lint_cmd}` 通过后，主 Agent **必须**在进入下一个子需求/下一波之前完成回写：
...
- **编码并行/串行由 Claude 判断**：并行仅限两个子需求同时进入 CODING（满足"互不依赖 + 目录不重叠（`{module_root_glob}/{service-name}/` 不重叠）"）；有依赖的按波次串行，前序产出（接口/{contract_type} 契约）注入后续 prompt
