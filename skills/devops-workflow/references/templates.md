...
```
另：`{讨论根目录}.workflow-active` 是活动上下文指针（单行 `{需求ID}[#里程碑]`，便捷用，非状态源）。

**产出物根路径约定**：阶段 2~5 产出物路径写作 `.task/{...}` 时，单里程碑指 `{讨论根目录}{域}/{需求名}/.task/{...}`；多里程碑下除讨论文档、架构时序图、`design-foundation.md` 等需求级共享物外，分析/设计/任务/审查一律落到 `.task/milestones/{里程碑}/{...}`，各 agent 工作范围限定在该里程碑覆盖的模块。两种模式执行逻辑一致，仅根路径不同。
...
> 每个任务的完成定义（DoD）取决于复杂度级别：
> - **简单任务**（`simple_task_skip_cr=true` 时）：代码写完 + 单测写完 + 测试通过 + 语法/编译检查通过 → DONE（跳过 CR）
> - **普通/复杂任务**：代码写完 + 单测写完 + 测试通过 + 语法/编译检查通过 + CR 扫描 + **CR 问题人工确认** + 已采纳问题改写完成 + 复验通过 → DONE
> 测试命令和检查命令按项目 `.claude/rules/` 定义执行。静态分析是否纳入 DoD 由项目规则决定。
> 任务状态流转：`TODO →（复杂：PLANNING → PLAN_CONFIRMED →）CODING →（简单：直接 DONE | 普通/复杂：CR_SCANNED → CR_CONFIRMED → REWRITING → VERIFYING → DONE）`
- [ ] {X.Y} {范围标签} · {任务标题} [{简单|普通|复杂}] — 状态: {TODO|PLANNING|PLAN_CONFIRMED|CODING|CR_SCANNED|CR_CONFIRMED|REWRITING|VERIFYING|DONE} — plan: .task/plans/{范围标签}-{X.Y}.md — CR: .task/review/{范围标签}-{X.Y}.md

## 返工记录（无返工时省略；由 /devops-workflow rework 追加）
| 轮次 | 日期 | 根因层级 | 影响任务 | 返工单 |
|------|------|---------|---------|--------|
...
- [ ] {X.Y} {范围标签} · {任务标题} [{简单|普通|复杂}] — 状态: {TODO|PLANNING|PLAN_CONFIRMED|CODING|CR_SCANNED|CR_CONFIRMED|REWRITING|VERIFYING|DONE} — plan: milestones/{里程碑A}/plans/{范围标签}-{X.Y}.md — CR: milestones/{里程碑A}/review/{范围标签}-{X.Y}.md
