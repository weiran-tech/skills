...
子需求 ID 完整格式：`{service-name}/{父需求名}#{子需求名}`

**按最后一个 `#` 拆分**：
- `#` 之前 = service-name/父需求路径（含 `/`，多段路径整体保留）
- `#` 之后 = 子需求名（单段）
...
子需求目录：`docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/metadata.md`
...
## 基本信息
- subReqId: {service-name}/{父需求名}#{子需求名}        # 完整 ID（含 #）
- parentReqId: {service-name}/{父需求名}                # 父需求 ID（不含 # 和子名）
...
|------|---------|----------------------------------|
| 1 需求讨论 | 讨论文档 | docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/discussion.md |
...
## 子需求列表
subRequirements:
- {service-name}/{父需求名}#{子需求名1}
- {service-name}/{父需求名}#{子需求名2}
- ...
...
```
docs/discuss/{service-name}/{父需求名}/
└── .task/
    └── {子需求名}/
...
```
docs/
├── discuss/                          # 子需求树（按 service-name/父需求/子需求组织）
│   └── {service-name}/{父需求名}/.task/{子需求名}/...
└── version/                          # 版本聚合（全局，跨父需求/跨 service-name）
    ├── v1.0                          # 无扩展名
    ├── v1.1
    └── v2.0
```
...
| 资源类型 | 根路径 |
|---------|--------|
| 子需求元数据 | `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/metadata.md` |
| 子需求阶段产物 | `docs/discuss/{service-name}/{父需求名}/.task/{子需求名}/{阶段产物}` |
