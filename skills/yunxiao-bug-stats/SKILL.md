...
| `space_id`        | 云效项目ID                            | 是   | -                |
| `range`          | 统计日期范围，自然语言解析，如 `"昨天"`、`"最近7天"`、`"2026-05-01 - 2026-05-11"` | 否   | 当前月份            |
...
2. **`{range}`创建的线上故障总数量** - 按创建时间筛选，解析日期范围
3. **`{range}`关闭的线上故障总数量** - 状态阶段处于(正常结束/异常结束)且状态更新于日期范围内
...
```bash
uv run scripts/stats.py <json-file> "<range>" [organization_id]
```

stats.py 读取工作项列表并统计：
- 未解决：`status.id` 不在已关闭状态集合中
- 创建于范围内：`gmtCreate` 时间戳在日期范围内
- 关闭于范围内：`status.id` 在已关闭状态 且 `updateStatusAt` 时间戳在日期范围内
...
2. **按负责人分组的故障清单（必须实现）**：
   ...
   - 每组内使用 Markdown 表格展示，包含列：优先级、严重程度、状态、编号、标题、**创建时间**、链接
   - 每组按故障数量降序排列（故障多的负责人在前）
   - 优先级、严重程度、状态三列必须带 Emoji 标识；创建时间为 `YYYY-MM-DD HH:MM` 格式
