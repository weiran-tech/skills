...
```
阶段 1 需求讨论        /devops-workflow req create {域}/{父需求名}
                          ↓
                          /devops-workflow req split {域}/{父需求名}   （交互式向导逐个创建子需求 metadata.md）
                          ↓
...
| 动作                             | 触发                                       | 读取                                                 | 写入                                                          | 校验                         |
| -------------------------------- | ------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------------------- | ---------------------------- |
| 校验域合法性                     | `/devops-workflow req create {域}/{父需求名}` | `{discovery_cmd}` 域清单 / `docs/workflow/` 模块命名 | —                                                             | 域 ∈ manifest 声明的模块清单 |
| 校验父需求不存在                 | 同上                                       | `docs/discuss/{域}/{父需求名}/` 是否存在             | —                                                             | 不存在才能创建               |
| 创建父需求目录骨架               | 同上                                       | —                                                    | `docs/discuss/{域}/{父需求名}/.task/parent.md`                | —                            |
| 列出已存在子需求（用于重名检查） | `/devops-workflow req split {域}/{父需求名}`  | `.task/` 子目录扫描                                  | —                                                             | 用于交互式向导重名检查       |
...
| --------------------------- | ------------------------------------------------------------- | ---------------------------------------------- | ----------------------------------------------------------------------------------------- | --------------------- | ---------------------------- |
| N1 创建父需求               | `/devops-workflow req create {域}/{父需求名}`                    | manifest 模块清单                              | `docs/discuss/{域}/{父需求名}/.task/parent.md`                                            | —                     | 提示下一步 `req split`       |
| N2 拆分子需求（向导）       | `/devops-workflow req split {域}/{父需求名}`                     | `.task/` 子目录                                | 子需求目录 + `metadata.md`（8 字段）                                                      | —                     | 提示下一步 `version create`  |
...
| 命令                              | 必带 / 可选               | 关键效果                                                   |
| --------------------------------- | ------------------------- | ---------------------------------------------------------- |
| `/devops-workflow req create`        | `{域}/{父需求名}`         | 创建父需求 + parent.md                                     |
| `/devops-workflow req show`          | `{域}/{父需求名}`         | 显示父需求详情 + 子需求清单                                |
| `/devops-workflow req list`          | （无）                    | 列出所有父需求                                             |
| `/devops-workflow req split`         | `{域}/{父需求名}`         | 交互式向导逐个创建子需求                                   |
