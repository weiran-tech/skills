...
1. **解析子需求 ID**：格式 `{域}/{父需求名}#{子需求名}`（按最后一个 `#` 拆分；解析规则见 `templates.md §0`）。缺参 → 报错 + 列出可 rework 的子需求（详见 `commands.md §C rework`）。
...
6. **查找返工轮次 R{N}**：扫描 `docs/discuss/{域}/{父需求名}/.task/{子需求名}/rework/` 目录，取最大 `R{n}-` 编号 +1；首次返工 R1

7. **写返工单文件**：`docs/discuss/{域}/{父需求名}/.task/{子需求名}/rework/R{N}-{YYYY-MM-DD}.md`
   - 内容含：返工轮次、日期、根因层级、缺陷描述、初判受影响任务/模块、依赖扩散结果

8. **依赖扩散（自动算 + 人工确认）**：
   - 读 `docs/discuss/{域}/{父需求名}/.task/{子需求名}/dev-tasks.md` 依赖图
...
子需求依赖图存于 `docs/discuss/{域}/{父需求名}/.task/{子需求名}/dev-tasks.md`，含两类：
1. **任务级依赖**（当前子需求内）：任务 A 依赖任务 B
2. **子需求级依赖**（跨子需求）：子需求 X 依赖子需求 Y 的产物（如 Y 的 API、Y 的 DDL、通过 `{contract_type}` 通道的契约）
...
路径：`docs/discuss/{域}/{父需求名}/.task/{子需求名}/rework/R{N}-{YYYY-MM-DD}.md`
