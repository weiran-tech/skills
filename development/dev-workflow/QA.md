...
※ 任何时候变更 manifest / CLAUDE.md / docs/architecture/ → /dev-workflow discovery refresh
```
...
| 动作 | 触发 | 读取 | 写入 | 校验 |
|------|------|------|------|------|
| 校验域合法性 | `/dev-workflow req create {域}/{父需求名}` | `{discovery_cmd}` 域清单 / `docs/architecture/` 模块命名 | — | 域 ∈ manifest 声明的模块清单 |
...
| 出 plan（复杂） | PLANNING → PENDING_PLAN_REVIEW | `architect` / `planner`（只读） | `design-consensus.md` + `docs/architecture/{模块}` + 现有代码 | `.task/{子需求名}/plans/{序号}-{模块名}.md` | 6 项齐全（签名 / 数据模型 / 时序 / 边界 / 测试用例 / 迁移） |
...
| ③ CR 扫描 | CODING → CR_SCANNED | `code-reviewer`（只读） | `git diff <默认分支>...HEAD -- {module_root_glob}` + done.md + design-consensus.md + docs/architecture/{模块} | `.task/{子需求名}/review/{模块名}-{任务序号}.md`（编号清单 + 严重度 + 文件:行 + 建议改法 + 裁决:PENDING） | **绝不允许直接改代码** |
...
| 跨模块一致性 | `design-consensus.md` + `docs/architecture/cross-module.md` | — | {contract_type} 契约闭合 |
...
| 重新执行 §1 全部发现 | manifest 文件（mtime）/ CLAUDE.md（mtime）/ `docs/architecture/`（mtime） | 更新会话内存 DiscoveryContext | 三段式错误格式不变 |
...
|------|---------------------|----------------|
| `language` | CLAUDE.md → manifest#engines → docs/architecture 命名 | manifest / CLAUDE.md mtime 变更 |
| `test_cmd` / `lint_cmd` / `static_analysis_cmd` | CLAUDE.md → manifest scripts | 同上 |
| `module_root_glob` | CLAUDE.md → manifest workspaces → docs/architecture 命名 | 同上 + 新增模块 |
| `contract_type` | `docs/architecture/{module}/contracts.md` → CLAUDE.md → manifest | `docs/architecture/` mtime 变更 |
...
| 跨模块一致性（阶段 5） | 对照 design-consensus.md + `docs/architecture/cross-module.md` | {contract_type} 契约闭合 |
...
/dev-workflow discovery refresh
  ...
  → 重新从 manifest + CLAUDE.md + docs/architecture/ 解析
