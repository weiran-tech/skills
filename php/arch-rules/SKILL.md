...
docs/{module}/overview   → 事实（模块是什么、目录结构、依赖）     ← php-arch-analyzer 生成
docs/{module}/business   → 规则（为什么这么处理）                 ← php-arch-analyzer 生成
docs/{module}/contracts  → 契约（路由、事件、Job）                ← php-arch-analyzer 生成
docs/{module}/flows      → 链路（怎么流转）                       ← php-arch-analyzer 生成
...
- `docs/{module}/overview.md` → 模块职责、目录结构、技术栈、跨模块依赖
- `docs/{module}/contracts.md` → 事件契约、路由清单、Job 列表、跨模块调用
...
3. **`.claude/rules/cross-module-guide.md`** ← 读取 `references/cross-module-guide.md`
