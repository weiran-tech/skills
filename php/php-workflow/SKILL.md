...
面向伪多模块单体项目：代码同仓 `modules/{模块}`，跨模块走 Events/Listeners + Service，审查按模块路径过滤 diff（`git diff <默认分支>...HEAD -- modules/{模块}`），架构上下文来自 `docs/workflow/{service-name}/`（php-analyzer 产出）。测试/校验等具体命令默认按 PHP 工具链（phpunit / php -l），可按项目配置替换。
...
```
/php-workflow req create {域}/{父需求名}                    # 创建父需求（单子需求场景）
/php-workflow req show {子需求ID}                          # 查看子需求详情（metadata.md + 阶段产物）
/php-workflow req list                                    # 列出所有子需求（含状态/版本绑定）
/php-workflow req split {域}/{父需求名}                    # 把大需求拆成多个子需求（交互式向导）
```
