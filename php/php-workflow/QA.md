# php-workflow QA — 交互流程 / 文件追踪 / 节点清单

> 目的：把 `php/php-workflow/` 下的全部内容做一份"测试与排查手册"——读完它就能照着走完一遍流程、知道每个文件管什么、知道每个节点该读谁/写谁/校验谁。
>
> 本 QA 文档基于以下文件（2026-07-06 读取快照）：

| 路径 | 角色 |
|------|------|
| `SKILL.md` | 路由器 + 常驻安全规则 |
| `README.md` | 总体流程、设计理念、命令速查、使用示例 |
| `references/templates.md` | `progress.md` 模板、状态枚举、目录约定 |
| `references/commands.md` | 各 `/php-workflow` 子命令的详细处理逻辑 |
...
| `references/summary.md` | `/php-workflow summary` 交付清单（DDL / 队列 / API） |
...
└────────────────────────────────────────────────┘
   ▼
阶段 4 开发与逐任务审查   /php-workflow next
   │
   ▼
阶段 5 收尾验收        /php-workflow next
   │  (产出 acceptance.md)
   ▼
COMPLETED ──→ /php-workflow summary  →  change-manifest.md（DDL / Job·MQ / API）

※ 阶段 4/5 若发现设计/需求层缺陷 → /php-workflow rework 按根因层级回退
```
...
|----|---------|---------|---------|---------|
| 设计审核门 | `PENDING_DESIGN_REVIEW` | `/php-workflow approve` | design-consensus 必含清单逐项 ✓ | design-consensus.md 末尾追加 `## 设计确认: APPROVED`，状态置 DEVELOPING |
| 任务 plan 门（仅复杂任务） | `PENDING_PLAN_REVIEW` | `/php-workflow approve` | plan 必含项齐全 | plan.md 末尾追加 `## 设计确认: APPROVED`，任务状态置 PLAN_CONFIRMED |
| CR 问题裁决门（逐任务） | `PENDING_CR_REVIEW` | `/php-workflow approve` | 全部问题标注 ACCEPTED/REJECTED/MODIFIED | 任务状态置 CR_CONFIRMED → 走改写⑤ / 复验⑥ |
...
| 动作 | 触发 | 读取 | 写入 | 校验 |
|------|------|------|------|------|
| 询问业务域 | `/php-workflow start {需求名}` | `composer.json` `autoload.psr-4` / `modules/` 列表 | — | 必须唯一确定域 |
...
| 动作 | 触发 | 读取 | 写入 | 校验 |
|------|------|------|------|------|
| 提取受影响模块 | `/php-workflow next` | `docs/discuss/{域}/{需求名}.md` 影响分析章节 | `.task/progress.md` "影响模块" 字段 | — |
| 优先复用架构文档 | 同上 | `docs/workflow/{模块名}/{overview,business,contracts,flows}.md`、`docs/workflow/cross-module.md` | — | 文档缺失或过期时启动分析 |
| 逐模块分析（必要时） | 启动 `team {N}:analyst` | `modules/{模块}` 代码 + 架构文档 | `.task/analysis/{模块名}.md` | a/b/c 三段齐全 |
...
1. 对外契约（接口 / Event-Listener / 跨模块 RPC-Service）
2. 模块边界（谁该改 / 谁不该改）
3. 关键机制决策 + **取舍理由**
4. 验收标准
...
| 动作 | 触发 | 读取 | 写入 | 校验 |
|------|------|------|------|------|
| 定位目标 | `/php-workflow approve` | progress.md 当前状态 | — | 必须处于 `PENDING_DESIGN_REVIEW` |
...
| 出 plan（复杂） | PLANNING → PENDING_PLAN_REVIEW | `architect` / `planner`（只读） | design-consensus + `docs/workflow/{模块}` + 现有代码 | `.task/{...}/plans/{序号}-{模块名}.md` | 6 项齐全（签名 / 数据模型 / 时序 / 边界 / 测试用例 / 迁移） |
| 人工确认 plan | PENDING_PLAN_REVIEW → PLAN_CONFIRMED | — | plan.md | plan.md 末尾追加 `## 设计确认: APPROVED` | plan 不达标 → 打回重出 |
| ① 编码 | PLAN_CONFIRMED / TODO → CODING | `executor`（并行 `team` / 串行单） | design-consensus 或已确认 plan + 前序契约 | `modules/{模块}` 代码 + 单测 + `.task/done/{模块名}.md` | DoD：单测绿 + php -l 通过 |
| ② DoD 自检 | CODING | 同上 | 改动文件 | — | phpunit 绿 + php -l 通过 |
| ③ CR 扫描 | CODING → CR_SCANNED | `code-reviewer`（只读） | `git diff <默认分支>...HEAD -- modules/{模块}` + done.md + design-consensus + docs/workflow/{模块} | `.task/review/{模块名}-{任务序号}.md`（编号清单 + 严重度 + 文件:行 + 建议改法 + 裁决:PENDING） | **绝不允许直接改代码** |
| ④ 人工裁决 | CR_SCANNED → PENDING_CR_REVIEW | 人工 | 报告『人工裁决』区 | 报告『人工裁决』区填 ACCEPTED / REJECTED / MODIFIED | 全部 PENDING 必须变更为三选一 |
| 锁定裁决 + CR_CONFIRMED | PENDING_CR_REVIEW → CR_CONFIRMED | `/php-workflow approve` | — | progress.md 任务状态置 CR_CONFIRMED | — |
| ⑤ 改写（仅 ACCEPTED / MODIFIED） | CR_CONFIRMED → REWRITING | `executor` | 已确认问题清单 + 当前代码 | modules/{模块}/ | REJECTED 项禁止改动 |
| ⑥ 复验 + 进度回写 | REWRITING → VERIFYING → DONE | 主 Agent | — | progress.md（任务状态 + 里程碑进度计数）+ dev-tasks.md（子项 `[ ]→[x]`） | phpunit 绿 + php -l 通过 + 已采纳问题修复 |
...
| 全量回归 | `vendor/bin/phpunit` | — | 全绿 |
| 语法校验 | 改动文件 | — | `php -l` 全通过（phpstan 默认不纳入） |
| 跨模块一致性 | `.task/design-consensus.md` + `docs/workflow/cross-module.md` | — | Event/Listener、Service 调用契约闭合 |
| 迁移检查 | `modules/*/resources/migrations/` | — | 迁移与回滚可用 |
...
- 设计错 / 多任务受牵连 → `/php-workflow rework`（设计级）

### 2.6 收尾：`/php-workflow summary`

| 动作 | 读取 | 写入 | 校验 |
|------|------|------|------|
| 启动 `writer` / `document-specialist`（只读） | 本里程碑产物 + 定向 grep | `.task/[milestones/{里程碑}/]change-manifest.md` | — |
| 三大块汇总 | `modules/*/resources/migrations/` + 路由 + 控制器 + design-consensus 契约 | 同上 | DDL N 张表 / 队列 M 个 / API K 个 |
...
|------|---------|------|------|---------|-----------------|
| N1 设定活动上下文 | `/php-workflow use [ID][#M]` | 当前 `.workflow-active` | `docs/discuss/.workflow-active` | 覆盖 | 回显当前活动 + 阶段/状态 |
| N2 起需求 | `/php-workflow start {名}` | composer/modules、`.task/` 是否存在 | `docs/discuss/{域}/{需求名}.md`、`.task/progress.md`、`.workflow-active` | DISCUSSING → ANALYZING | 提示下一步 `next` |
| N3 拆分里程碑 | `/php-workflow split [ID] {列表}` | 阶段 1 文档 | `.task/milestones/{M}/`、`design-foundation.md`、progress.md（多里程碑） | — | 设活动上下文为前置里程碑 |
| N4 推进下一阶段 | `/php-workflow next [ID][#M]` | progress.md | 各阶段产物 + progress.md | 详见 §3.2 | 按 §1.2 流水线推进 |
| N5 确认人工门 | `/php-workflow approve [ID][#M]` | 当前状态 | design-consensus.md / plan.md / 任务状态 | 视门类 | 详见 §3.3 |
| N6 查看状态 | `/php-workflow status [ID][#M]` | progress.md（们） | — | — | 输出进度展示 |
| N7 列需求 | `/php-workflow list` | 所有 `.task/progress.md` | — | — | 仅未 COMPLETED |
| N8 返工 | `/php-workflow rework [ID][#M]` | 缺陷描述 + 依赖图 | `.task/.../rework/R{N}-...md`、design-consensus.md（设计级）、progress.md | 按层级回退 | 列出受影响任务等用户确认 |
| N9 出交付清单 | `/php-workflow summary [ID][#M]` | 本里程碑产物 + 实际代码 | `.task/[milestones/{M}/]change-manifest.md` | — | 输出 DDL N / 队列 M / API K |
...
| S4 编码并行 | DEVELOPING + 至少 2 任务无依赖 | `team {N}:executor` | design-consensus 或已确认 plan | modules 代码 + 单测 + `.task/done/{模块名}.md` | CODING → VERIFYING 待 CR |
...
- 指针只有需求而该需求是多里程碑时：
  ...
  3. 切换到别的里程碑用 `/php-workflow use #{里程碑}`（只敲 `#`）
...
docs/discuss/{域}/{需求名}/
  ...
  .task/
    ...
    change-manifest.md        # /php-workflow summary 交付清单
```
...
| 阶段 2 模块分析 | `php-analyzer` / `analyst` | 只读 | 读代码 + 读架构文档 → 产出模块分析 |
...
人工门：阶段 3 设计审核、阶段 4 plan 确认、阶段 4 CR 裁决（均为 `/php-workflow approve`）。
...
7. **设计/需求层缺陷走 rework**，不要硬塞进 CR 改写：CR 改写只解决任务内小问题；牵动设计或多任务用 `/php-workflow rework`。
8. **里程碑默认单个**：只有大需求才 `split`；跨模块交互一律 Service / Events，禁止直接访问他模块 Model/Repository。
9. **中断恢复**：流程可在任意阶段中断，下次 `/php-workflow next` 从 progress.md 恢复。
...
| 项 | 命令 | 通过标准 |
|----|------|---------|
| 单测 | `vendor/bin/phpunit modules/{模块}/tests` | 绿 |
| 语法 | `php -l <改动文件>` | 通过 |
| 全量回归（阶段 5） | `vendor/bin/phpunit`（全量） | 绿 |
| 跨模块一致性（阶段 5） | 对照 design-consensus + `docs/workflow/cross-module.md` | Event/Listener、Service 契约闭合 |
| 迁移（阶段 5，若涉及） | `modules/*/resources/migrations/` | 迁移与回滚可用 |
| phpstan | — | **默认不纳入 DoD / 验收**（存量项目历史告警多）；项目有干净基线时再可选开启 |
...
| 命令 | 必带 / 可选 | 关键效果 |
|------|------------|---------|
| `/php-workflow use [ID][#M]` | 可不带 → 显示当前；带 → 切换 | 写 `.workflow-active` |
| `/php-workflow start {需求名}` | 必带需求名 | 创建需求 + 设活动 + 进入阶段 1 |
| `/php-workflow split [ID] {M列表}` | 列表用逗号分隔 | 多里程碑结构 + 设前置里程碑为活动 |
| `/php-workflow next [ID][#M]` | 可省 | 推进到下一阶段 / 下一个任务 |
| `/php-workflow approve [ID][#M]` | 可省 | 按当前状态分发：设计审核 / plan / CR 裁决 |
| `/php-workflow status [ID][#M]` | 可省 | 单里程碑简版；多里程碑表 + 高亮 |
| `/php-workflow list` | 无 | 未完成需求 + 多里程碑子简况 |
| `/php-workflow rework [ID][#M]` | 可省 | 缺陷返工（需用户选根因层级） |
| `/php-workflow summary [ID][#M]` | 可省 | 产出 change-manifest.md + 输出条数摘要 |
...
/php-workflow next       # 阶段 5：verifier 全量 phpunit + php -l + 跨模块一致性
  → 通过 → COMPLETED

/php-workflow summary    # 产出 change-manifest.md（DDL/Job·MQ/API）
```
...
/php-workflow next        # 阶段 2 → PENDING_DESIGN_REVIEW
/php-workflow approve     # 设计审核通过
/php-workflow next        # 阶段 4：某任务判为【复杂】→ architect 出 plan，停在 PENDING_PLAN_REVIEW
/php-workflow approve     # plan 确认 → 编码 → CR → PENDING_CR_REVIEW ...
...
/php-workflow rework payment/支付渠道重构#alipay
  → 确认根因 = 设计级
...
/php-workflow next payment/支付渠道重构#alipay     # 阶段 2 修订 design-consensus
/php-workflow approve payment/支付渠道重构#alipay  # 阶段 3 重审通过
/php-workflow next payment/支付渠道重构#alipay     # 复杂任务重出 plan → 重 code → 重 CR
```
...
- [ ] 是否调用 `/php-workflow approve`（而非 `next`）
...
- [ ] 子 agent 返回后主 Agent 是否输出了 `/php-workflow approve` 提示字符串（漏了说明漏了第 3 步）
...
- [ ] 并行 executor 是否限定了模块目录（`modules/{模块}/`）
...
- [ ] 只改当前任务能解决 → 用 CR 改写
- [ ] 牵动设计或别的任务 → 用 `/php-workflow rework`
...
- [ ] 切换里程碑用 `/php-workflow use #{M]` 而非长 ID
...
| 执行 `/php-workflow summary` 产出交付清单 | `references/summary.md` |
