...
|------|---------|------|--------------|
| `/php-workflow req create` | `{域}/{父需求名}` | 创建父需求目录骨架 | 替代 `start` |
| `/php-workflow req show` | `{域}/{父需求名}` | 显示父需求详情 + 子需求清单 | — |
| `/php-workflow req list` | （无） | 列出所有父需求 | — |
| `/php-workflow req split` | `{域}/{父需求名}` | 交互式向导逐个创建子需求 | 替代 `split` |
...
| `/php-workflow start {需求名}` | ❌ 已删除 | **`/php-workflow req create {域}/{父需求名}`** —— 创建父需求后，用 `req split` 进入子需求创建向导 |
