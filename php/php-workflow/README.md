...
| **三道人工门**             | 设计审核（阶段3）、复杂任务 plan（阶段4）、CR 问题裁决（阶段4）——关键决策必须人确认 |
...
```
 ...
 └──────────────────────────────────────────┘
  ...
  阶段4 开发与逐任务审查  /php-workflow next  （见下方"任务级闭环"）
     │
     ▼
  阶段5 收尾验收        /php-workflow next   → verifier 全量回归 + 一致性把关
     │ (产出 acceptance.md)
     ▼
...
| 需求级     | 需求本身错          | 回阶段1 重新 /req-discuss              | 视讨论结论，可能整体重来                                              |
...
| `/php-workflow approve [需求ID][#里程碑]`   | 确认当前人工门（设计审核 / plan / CR 裁决，按状态自动分发）     |
...
# 修订设计 → 重审 → 受影响任务级联重跑
/php-workflow next payment/支付渠道重构#alipay     # 阶段2 修订 design-consensus
/php-workflow approve payment/支付渠道重构#alipay  # 阶段3 重审通过
