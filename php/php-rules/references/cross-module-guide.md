...
| 文档 | 什么时候看 |
|------|-----------|
| `docs/workflow/{module}/contracts.md` | 新增/修改 Event 或跨模块调用前（避免事件冲突、参数不兼容） |
| `docs/workflow/{module}/flows.md` | 评估改动的下游影响范围时 |
| `docs/workflow/{module}/business.md` | 实现业务逻辑前（理解状态机和业务规则） |
| `docs/workflow/{module}/overview.md` | 初次进入某个模块时 |
